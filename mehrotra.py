...
from ldlt_solver import LDLT_solver
...
class IPSolver():
    ...
    def solve_QP(self, Q: np.ndarray, c: np.ndarray, x_init: np.ndarray, tol_r: float=1e-8, A: np.ndarray=None,
                 ...
                 u_init: np.ndarray=EMPTY, s_init: np.ndarray=EMPTY, t_init: float=EMPTY, red_t: float=0.3,
        ...
        for i in range(max_iter):
            ...
            # dz = np.linalg.solve(M, - r)
            # print(dz)
            dz = solver.solve_via_LDLT(Q, A, C, s_k, u_k, r)
            # print(dz)
